...
def execute():
    ...
    for what should be a single department (DSAC is the correct spelling).
    ...
    This merges ABA into BTS, merges DSAIC into DSAC, and carves the
    ...
    if frappe.db.exists("Division", "DSAIC") and frappe.db.exists("Division", "DSAC"):
        frappe.rename_doc("Division", "DSAIC", "DSAC", merge=True, force=True)

    if not frappe.db.exists("Division", "GPS"):
        frappe.get_doc({
            "doctype": "Division",
            "division": "GPS",
            "description": "Split out from DSAC: courses previously branded FESGI, now GPS."
